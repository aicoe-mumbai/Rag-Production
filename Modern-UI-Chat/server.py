...
def is_port_in_use(port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        return s.connect_ex(('172.16.34.235', port)) == 0
...
# Function to extract text from PDF using OCR when needed
def extract_text_from_pdf(pdf_file):
    ...
    try:
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            # If page has no text or very little text, we'll use OCR
            if not page_text or len(page_text.strip()) < 100:
                print(f"Page {page_num+1} has little or no text, attempting OCR")
                # Reset file pointer for pdf2image
                pdf_file.seek(0)
                # Only convert the specific page that needs OCR
        ...
        # If direct extraction fails, try OCR on all pages
        try:
            pdf_file.seek(0)
...
def process_document():
    ...
    try:
        ...
        if file_type in ['csv', 'xlsx']:
            ...
            print("Processing PDF file with OCR support")
            # Use the new PDF extraction function
            original_content = extract_text_from_pdf(file_data)
...
if __name__ == '__main__':
    ...
    port = 8444  # Fixed to port 8444 only
    ...
    try:
        ...
        print("Please ensure port 8444 is not in use by another process.")
        print("You can kill processes using this port with: lsof -i :8444 | awk 'NR>1 {print $2}' | xargs kill -9")
        sys.exit(1) 